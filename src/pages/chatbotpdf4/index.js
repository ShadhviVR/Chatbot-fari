...
const ChatInterface = () => {
  ...
  useEffect(() => {
    const initiateConversation = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/initiate?pdf_url=https://memberfiles.freewebs.com/23/04/36560423/podcast/ACT%2001.pdf`);
  ...
  const handleRefresh = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/initiate?pdf_url=https://arxiv.org/pdf/2307.09951.pdf`);
      setConversationId(response.data.conversation_id);
      setMessages([]);
    } catch (error) {
      console.error('Error initiating conversation:', error);
